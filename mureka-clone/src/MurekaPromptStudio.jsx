...
export default function MurekaPromptStudio({ apiBase, apiKey, onOpenKeys, onStudioPulse, onGoLocalWithLyrics, onSongReady }) {
  ...
  }, [apiBase, apiKey, genre, mood, onOpenKeys, onSongReady, onStudioPulse, tempo, userPrompt, vocal])
    ...
    <div className="mps-page">
      ...
      <div className="mps-hero">
        ...
        <p className="mps-tagline">
          <strong>Real vocals from your words</strong> — paste lyrics or a style prompt; this gateway sends them to{' '}
          <strong>Mureka</strong> (<code>/api/mureka/*</code>) so you get trained-model singing and production audio, not
          a browser demo or simple sine-wave placeholder. Same portal handles keys, retries, and same-origin speed when
          you deploy the full stack.
        </p>
        ...
        <div className="mps-card mps-card--primary">
          ...
          <input
            ...
            placeholder="Paste your lyrics, or describe the track — Mureka renders real performed vocals (not local synth)"
            maxLength={500}
            disabled={busy}
          />
        ...
        <div className="mps-card mps-card--local-bridge">
          <h3 className="mps-local-bridge-title">Optional · Local beat lab (draft vocal)</h3>
          <p className="mps-local-bridge-hint">
            <strong>Local → Make Song</strong> mixes your beat with a <strong>procedural</strong> vocal stem for timing
            tests—not Mureka’s AI singers. For <strong>pure vocal tone</strong>, stay here and{' '}
            <strong>Generate with Mureka</strong> with your lyrics above (or use <strong>Voice studio</strong> with Mureka
            + beat).
          </p>
          <button
            type="button"
      ...
      <section className="mps-features">
        <div className="mps-fcard">
          ...
          <h3>Single portal</h3>
          <p>
            Browser → <code>/api</code> → Mureka and optional OpenAI / Claude (Anthropic) for lyrics. Keys stay behind the
            server
